...
import 'package:demo1/models/class_history_cuti.dart';
import 'package:demo1/pages/detail_cuti_page.dart';
import 'package:demo1/widgets/data_history_cuti.dart';
...
class HistoryCutiPage extends StatefulWidget {
  final String nip;
  const HistoryCutiPage({
    super.key,
    required this.nip,
  });

  @override
  State<HistoryCutiPage> createState() => _HistoryCutiPageState();
}

class _HistoryCutiPageState extends State<HistoryCutiPage> {
  final logger = Logger(
  ...
  late Future<List<HistoryCuti>> dataHistoryIzin;
  ...
  Future<List<HistoryCuti>> fetchHistoryData() async {
    final response = await http.get(
      Uri.parse(
        'https://alterindo.com/hris/api.php?action=data_pengajuan_cuti&Kode=${widget.nip}&TanggalAwal=${DateFormat('yyyy-MM-dd').format(firstDate)}&TanggalAkhir=${DateFormat('yyyy-MM-dd').format(dateNow)}',
      ),
      headers: {
        'Authorization': 'Bearer ${API.token}',
      },
